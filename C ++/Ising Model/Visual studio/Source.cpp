...
int main(int argc, char **argv) {
	...
	char name1[11] = "sweeps.txt";
	sys->openFile1(name1);

	char name2[11] = "netmag.txt";
	sys->openFile2(name2);

	char name3[11] = "dimE.txt";
	sys->openFile3(name3);
...
void drawFuncs::introMessage() {
	...
	cout << " press m for the magnetisation of the current system" << endl;
}
...
void drawFuncs::handleKeypress(unsigned char key, int x, int y) {
	...
	case '?':
		...
		break;
	case 'm':
		cout << "Magnetisation is: " << sys->M << endl;
		sys->Netmag();
		break;
	}
	glutPostRedisplay();
