...
void IsingSystem::MCsweep() 
{
	...
	Netmag();
	DimE();
	if(CurrentSweep == 1 && logfile1.is_open()){logfile1 << CurrentSweep << endl;}
	if(CurrentSweep == 1 && logfile2.is_open()){logfile2 << M << endl;}
	if(CurrentSweep == 1 && logfile3.is_open()){logfile3 << EJ << endl;}
	if(CurrentSweep %10 == 0 && logfile1.is_open()){logfile1 << CurrentSweep << endl;}
	if(CurrentSweep %10 == 0 && logfile2.is_open()){logfile2 << M << endl;}
	if(CurrentSweep %10 == 0 && logfile3.is_open()){logfile3 << EJ << endl;}
...
{	
	if (CurrentSweep < MCmax)
	{
		MCsweep();
	}
	else if (CurrentSweep == MCmax && RepeatNo < 10)
	{
		Reset();
		cout << "Simulation Iteration number" << RepeatNo << endl;
		RepeatNo++;
	}
}

void IsingSystem::Netmag()
{
	double summation = 0; 
	for (int i = 0; i < gridSize; i++) // loop over all rows
	{
		for (int j = 0; j < gridSize; j++) // loop over all coloumns
		{
			int pos[2] = { i,j };          //
			summation += readGrid(pos); // running total of spin sums
		}
	}
	M = summation / (gridSize*gridSize); // equation 2 from coursework handout
}

void IsingSystem::DimE()
{
	double sum = 0;
	// loop over all grid sites
	for (int i = 0; i < gridSize; i++)
	{
		for (int j = 0; j < gridSize; j++)
		{
			int pos[2] = { i,j }; // check each spin site
			for (int n = 0; n < 4; n++)  // loop over all neighbours
			{ 
				int nborPos[2]; // neighbour positions stored in array
				setPosNeighbour(nborPos, pos, n); 
				sum += -(readGrid(nborPos)*readGrid(pos));
			}
		}
	}
	EJ = sum / (gridSize*gridSize);
}
